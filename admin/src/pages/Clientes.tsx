...
export default function Clientes() {
  ...
  const baseFiltrada = registroId
    ? clientes.filter(c => String(c._id || c.id) === registroId)
    : clientes
  const filtrados = busca
    ? baseFiltrada.filter(c => {
      const termo = busca.toLowerCase()
      const numeros = normalizarNumero(busca)
      return c.name?.toLowerCase().includes(termo)
        || c.email?.toLowerCase().includes(termo)
        || c.phone?.toLowerCase().includes(termo)
        || (numeros && normalizarNumero(c.phone).includes(numeros))
        || (numeros && normalizarNumero(c.cpf).includes(numeros))
    })
    : baseFiltrada
      ...
      <div className="flex-1 p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4 items-start sm:items-center justify-between">
          <div className="glass-card flex items-center gap-2 px-3 py-2 flex-1 max-w-md">
            ...
            <input type="text" placeholder="Buscar..." value={busca} onChange={e => { setBusca(e.target.value); setRegistroId('') }} className="bg-transparent text-white text-sm w-full outline-none placeholder-slate-600" />
          </div>
